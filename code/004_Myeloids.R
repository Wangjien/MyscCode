...
# scRNA <- SCP::Integration_SCP(scRNA, batch = "sample", integration_method = "Harmony", cluster_resolution = seq(0.1, 1.5, 0.1))
# qsave(scRNA, file = "./01_髓系数据_scp.qs")
# BBKNN聚类
scRNA <- SCP::Integration_SCP(scRNA, batch = "sample", integration_method = "BBKNN", cluster_resolution = seq(0.1, 1.5, 0.1))
qsave(scRNA, file = "./01_髓系数据_scp.qs")
# scVI聚类
...
p2 <- DimPlot(scRNA,
  reduction = "BBKNNUMAP2D", group.by = "BBKNN_res.1.5", label = T, repel = T, label.size = 6,
...
p3 <- DimPlot(scRNA, group.by = "group1", reduction = "BBKNNUMAP2D", label = F, repel = T, cols = c(
  RColorBrewer::brewer.pal(n = 12, name = "Paired"),
  RColorBrewer::brewer.pal(n = 8, name = "Set2"), RColorBrewer::brewer.pal(n = 6, name = "Dark2")
)) +
  guides(color = guide_legend(override.aes = list(size = 6))) +
  ...
  theme(
    plot.title = element_text(hjust = 0.5, size = 18, color = "black"), legend.text = element_text(size = 10, color = "black"),
...
p4 <- DimPlot(scRNA,
  group.by = "celltype2", label = F, repel = T, raster = F, split.by = "group1", ncol = 6, reduction = "BBKNNUMAP2D",
...
FeaturePlot(scRNA,
  features = "ZMIZ1", reduction = "scVIUMAP2D", pt.size = 0.0001, max.cutoff = 1.5, split.by = "group", ncol = 5,
  cols = c("#FFEFD5", "#E6E6FA", "#87CEFA", "#6495ED", "#4169E1", "#0000CD", "#000080")
)